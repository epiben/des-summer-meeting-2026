...
1.  Expecting a mortality risk of 20% in patients with chronic mesenteric
    ...
    ggplot() +
        geom_density(aes(x = rbeta(100000, 9, 38)))

    find_beta_params(0.2, 0.1, "lower", n_dec = 2)

    mean(rbeta(100000, 9, 38) >= 0.1)
...
```{r}
ggplot() +
    geom_density(aes(x = rbeta(10000, 0.5, 0.5)))
```
...
day. Let's load the relevant packages and create a dataset that can be
...
3.  Compute the maximum a posteriori (MAP = the mode of the posterior distribution) and median posterior on the probability scale.
...
7.  Prior predictive checks are like posterior predictive checks, only that they ignore the data and generate data based solely on the priors. This can often be useful to check that the priors you have specified will yield data that look reasonable for the modelling task at hand. Carry out a prior predictive check of the three models above (default prior, `normal(0.0, 0.1)` and your own). All good?
...
1.  A friend of yours wants to test a new pre-stent treatment obelixaban
    ...
    evidence suggest that the 30-day mortality in the patient group
    receiving placebo is roughly 20% (95% CrI: 5%-35%). Your
...
5.  Compute average treatment effect using G-computation
    ...
    -   Hint: predict the posterior probabilities of death by day 90 for all patients if
...
- Article - John K. Kruschke: Bayesian Analysis Reporting Guidelines [https://www.nature.com/articles/s41562-021-01177-7](https://www.nature.com/articles/s41562-021-01177-7)
...
- R packge - Paul Buerkner: Bayesian regression models using Stan [https://paulbuerkner.com/brms/](https://paulbuerkner.com/brms/). Much more flexible R interface to Stan than rstanarm, and especially far superior if you need multi-level/hierarchical models or priors with more "unorthodox" shapes.
- Conjugate priors - Conjutate priors for a bunch of likelihood functions for when unadjusted analyses are enough [https://en.wikipedia.org/wiki/Conjugate_prior](https://en.wikipedia.org/wiki/Conjugate_prior)
...
- Book - Andrew Gelman et al.: Bayesian Data Analysis. The Bible but really not accessible and in no way the place to start for most people.

## G. Session info
This repo uses `renv` for reproducibility. Here's the session info:
